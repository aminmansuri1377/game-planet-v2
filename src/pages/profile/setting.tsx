...
function Setting() {
  ...
  const { data: buyerData, refetch } = trpc.main.getBuyerProfile.useQuery(
    { userId: userId! },
    { enabled: !!userId }
  );
  ...
  useEffect(() => {
    if (
      ...
      Array.isArray(buyerData.profileImage) &&
      buyerData.profileImage.length > 0
    ) {
      setCurrentProfileImage(buyerData.profileImage[0]);
    }
  }, [buyerData]);
  ...
  const onSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await photoBuyerProfile.mutateAsync({
        userId: userId!,
